...
function 公务员职位池(g: GameState, idx: number): PoolPos[] {
  ...
  if (idx <= 上限) {
    ...
    const 目标 = 序平台[Math.min(3, 现序 + 1)]
    const 城市s = shuffle(平台表[目标].城市)
    for (const c of 城市s.slice(0, 2)) {
      out.push(...政治本地职位(目标, idx, c).map((p) => ({ 名: p.名, 序号: Math.min(3, 现序 + 1), 本地: false, 城市: c, 二线: p.二线 })))
    }
  }
  return 补齐(out, g.p.城市, 现序)
}
...
function 补齐(pool: PoolPos[], 城市: string, 现序: number): PoolPos[] {
  const seen = new Set<string>()
  ...
  // 保证至少 7 个选项
  if (out.length < 7 && 城市) {
    const fillers = ['委办公室调研员', '政府办公室调研员', '人大常委会调研员']
    for (const f of fillers) {
      if (out.length >= 7) break
      out.push({ 名: `${城市}${f}`, 序号: 现序, 本地: true, 城市 })
    }
  }
  return out
}
